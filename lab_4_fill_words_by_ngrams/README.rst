...
2. Тест на уровень владения английским языком (TBA).
...
:py:class:`lab_4_fill_words_by_ngrams.main.WordProcessor`, который вы реализуете в
ходе выполнения первого шага. Данный класс необходимо наследовать от
...
:math:`\sum_{i=1}^{N}p(w_{i}|context) \leq P`
...
``[she', 'is']``, так как их суммарная вероятность равна `0.77`, что не превышает
порога `0.8`, и включение следующего наиболее вероятного токена ``'that'`` поднимет
суммарную вероятность выше заданного порога.
...
от принятия исходной последовательности до постобработки результата.
...
3. Выявить `N` наиболее вероятных токенов, чья суммарная последовательность
   максимально приближается к заданному `P`, не превышая его.
...
1. В случае, если буквы-кандидаты не были найдены, генерация прекращается.
...
Пусть размер n-грамм будет равен 7, длина последовательности - 51,
а значение `P` - 0.5.
...
противоположному усредненной сумме логарифмированных вероятностей
...
Атрибут, хранящий в себе значение метрики перплексии является защищенным: мы не предполагаем
...
3. Полученные вероятности необходимо использовать для подсчета `Perplexity` по формуле, приведенной
   выше.
...
``(1, 2, 3, 0)``. Здесь идентификатором токена конца предложения выступает 0.
...
Порядок возвращаемых значений должен быть отсортирован по значению метрики в порядке
возрастания.
...
в котором происходит чтение файла из соответствующего атрибута и обработка
его содержимого.
...
Метод должен сопоставить полученные ответы правильным и посчитать долю правильных ответов.
...
Шаг 5.5. Объявление сущности студента: алгоритмы генерации
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Шаг 5.6. Составление ответов на задания: алгоритмы генерации
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Шаг 5.7. Запрос типа алгоритма заполнения: алгоритмы генерации
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
.. important:: Выполнение Шага 6 соответствует 10 баллам.
