...
выделить два корня (`far` и `old`), а также суффиксы сравнительной степени `er` и `est`.
...
Один студент может отгадать не более одной загадки, таким образом, бонус cмогут получить
только 5 студентов.
